...
%
	...
	\begin{tikzpicture}[scale=1.5]
		...
		% droite base
		\draw[-, thick, black, dotted] (8,0) -- (9,0);
		
		% angle droit
		\draw[-, thick, black] (9,.15)-- (8.85,.15) -- (8.85,0);
		
		%hauteur
		\draw[-, thick, black, dotted] (9,0) -- (9,2) node[pos=.35, right=8pt, rotate=90] {hauteur};
	\end{tikzpicture}
	...
	\begin{tikzpicture}[>=stealth, scale=1]
		\begin{axis}[xmin = 0, xmax=1.1, ymin=0, ymax=1.1, axis x line=middle, axis y line=middle, axis line style=->, grid=both]
			...
			\addplot[black, dotted, thick] expression[domain=0:0.625, samples=2]{0.7805/0.625*x}
			node[midway, above=]{$1$};
			
			\addplot[no marks, GREEN_E, -, very thick] expression[domain=0.11:0.1732, samples=101]{sqrt(.03-x^2)}
			node[right, pos=.3] {$\theta$};
			
			\draw[black, dotted, thick] (axis cs:0.625, 0) -- (axis cs:0.625,0.7805);
