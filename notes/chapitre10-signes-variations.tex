...
\dfn{Signe d'un nombre réel}{
	...
	Il est préférable de dire \emph{positif ou nul} pour éviter tout ambiguïté.
}{}

\nt{
	Pour simplifier la notation \og $A >0$ et $B > 0$ \fg, on écrira juste $A, B > 0$.
}{}
...
\ex{Problème de racines}{
	...
	Plusieurs stratégies s'offrent à nous pour résoudre $f(x)=0$ : 
		\begin{itemize}
			\item Tracer $\C_f$ et lire les racines graphiquement. Cette méthode est utile pour approximer, mais ne permet pas d'obtenir les solutions exactes (ni le nombre de solution, d'ailleurs).
...
\ex{Problème de signe}{
	On souhaite connaître pour quels $x\in\R$ l'égalité suivante est vérifée.
...
\ex{Variation d'une fonction affine}{
	...
	Le tableau n'est pas très intéressant car les variations des fonctions affines ne le sont pas particulièrement...
}{}
