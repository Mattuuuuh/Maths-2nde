...
\ex{}{
	Considérons la fonction $f$ donnée par
		\[ f(x) = 2 \qquad \text{pour tout $x \in \R$.} \]
	Cette fonction est affine, car c'est un cas spécial de la forme \ref{eq:affine} où $a=0$ et $b=2$.
	La droite $(d) = \C_{f}$ vérifie donc
		\begin{align*}
			(x;y) \in (d) && \iff && y = 2,2.
		\end{align*}
...
On distingue trois cas différents de droites parmis les exemples précédents : les deux premières ont l'air de ``monter'', la suivante est constante, et la dernière a l'air de ``descendre''.
...
\pf{Démonstration du théorème \ref{thm:affine-var}}{
	...
	Sinon, on utilise les règles de manipulation des inégalités du théorème \ref{thm:ineg} vu au chapitre \ref{chap:3}.
...
\exe{Approfondissement}{
	...
	Grapher la parabole, par exemple en écrivant \texttt{y=x² -2x + 1} sur GeoGebra. \\ \href{https://www.geogebra.org/calculator}{https://www.geogebra.org/calculator}
