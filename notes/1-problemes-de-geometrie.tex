...
\nomen{
	On appelle $a + b \sqrt{d}$ l'\emphindex{expression conjuguée} de $a - b \sqrt{d}$.
}
...
\thm{de Dirichlet\footnotemark}{
	Pour tout nombre $x \in \R$, il existe un nombre infini de $p, q\in\Z$ vérifiant
		\[ \left|x - \dfrac{p}q \right| < \dfrac1{q}. \]
}{thm:Dirichlet}

\footnotetext{Johann Peter Gustav Lejeune Dirichlet (1805-1859), mathématicien allemand.}

\exe{, difficulty=2}{
	Soit $x\in\R-\Q$. Considérons $\{ 0, x, 2x, 3x, ..., qx \}$ les $q+1$ premiers multiples de $x$.
	Montrer qu'il existe nécessairement deux entiers naturels $m<n$ tels que $|nx - mx|$ ait une partie décimale comprise entre $0$ et $\dfrac1q$.
