...
\pf{}{
	Supposons que $\dfrac10 = x$ pour un certain nombre réel $x\in\R$.
...
\exe{}{
	À l'aide des exercices \ref{exe:graph-inv-pos} et \ref{exe:inv-impaire}, grapher la fonction inverse sur le domaine $\D = [-3 ; 0[ \cup ]0 ; 3]$.
}{exe:graph-inv-all}{
	\begin{center}
...
\exe{}{
	Montrer que si $x\in\Q^*$ est rationnel non nul, alors $\dfrac1x$ aussi.
}{exe:inv-Q-to-Q}{
	Si $x = \dfrac{a}b$ avec $a, b \in\Z^*$, alors $\dfrac1x = \dfrac{b}a \in\Q^*$.
}

\exe{}{
	Soit $f(x) = \dfrac1x$ la fonction inverse.
...
\exe{, difficulty=2}{
	Uniquement à l'aide des exercices \ref{exe:inv-Q-to-Q} et \ref{exe:inv-involution}, montrer que si $x\in\R^*-\Q$ est irrationnel non nul, alors $\dfrac1x$ aussi.
}{exe:inv-R-to-R}{
	Notons $f(x) = \dfrac1x$ la fonction inverse.
