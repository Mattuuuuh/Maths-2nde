...
\pf{Démonstration de la proposition \ref{prop:milieu-inter}}{
	...
	Celui-ci est à équidistance de $a$ et de $b$ et vérifie donc
		\[ |x-a| = |x-b|. \]
	Comme $x$ se situe entre $a$ et $b$, et par définition de la valeur absolue, on a donc
		\begin{align*}
...
\exe{}{
	...
	\begin{enumerate}
		\item Si les deux notes ont le même coefficient $\dfrac12$, quelle est la moyenne et donc le milieu de l'intervalle ?
		\item Posons $\lambda \in [0;1]$ le coefficient de la première note, et $1-\lambda$ celui de la deuxième.
...
\pf{Démonstration de la proposition \ref{prop:milieu-segment}}{
	...
	Ainsi ses coordonnées sont les moyennes des coordonnées et $A$ et de $B$.
		...
		\begin{tikzpicture}[>=stealth, scale=1]
		\begin{axis}[xmin = -3, xmax=8, xticklabel=\empty, ymin=-2.9, ymax=4, yticklabel=\empty, axis x line=middle, axis y line=middle, axis line style=<->, xlabel={}, ylabel={}]
		
			
			\addplot[black, mark=*, mark size = 1] (1,3) node[above, right] {$A$};
			\addplot[black, mark=*, mark size = 1] (7,-1) node[above, left] {$B$};
			\addplot[red, mark=*, mark size = 1] (4,1) node[above, left] {$M$};
			
			
			\addplot[black, mark=+, mark size = 1] (1,0) node[above] {$x_A$};
			\addplot[black, mark=+, mark size = 1] (7,0) node[below] {$x_B$};
			\addplot[black, mark=+, mark size = 1] (0,3) node[left] {$y_A$};
			\addplot[black, mark=+, mark size = 1] (0,-1) node[left] {$y_B$};
			
			
			\addplot[red, mark=+, mark size = 2] (4,0) node[below=10pt] {$\dfrac{x_A+x_B}2$};
			\addplot[red, mark=+, mark size = 2] (0,1) node[left] {$\dfrac{y_A+y_B}2$};
...
\nt{
	...
	Faites un dessin pour vous en assurer.
	
	Le milieu $M$ du segment est alors $\dfrac{A+B}2 = (-1;0)$, ce qui correspond au fait que le milieu de l'intervalle soit $-1$.
}

\thm{Longueur d'un segment}{
	Soient $A(x_A, y_A)$, $B(x_B, y_B)$ deux points du plan.
