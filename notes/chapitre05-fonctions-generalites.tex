...
\end{mintedbox}
		...
		Considérons la représentation graphique suivante d'une fonction $f$ définie sur $\D = ]{-}3,4 ; 2,3[$.
			...
			\begin{tikzpicture}[>=stealth]
				\begin{axis}[xmin = -3.4, xmax=2.3, ymin=-5.1, ymax=5.1, axis x line=middle, axis y line=middle, axis line style=->, grid=both]
					\addplot[no marks, blue, -] expression[domain=-4:3, samples=50]{x^3 /3 - 2*x +3}node[pos=.5, right]{$\C_f$};
				\end{axis}
			\end{tikzpicture}
			\end{center}
		\begin{enumerate}
	...
	\ex{}{
		...
		Cependant, si on arrive à démontrer que
			...
			\end{align*}
		et ceci pour tout $x\in\R$ nombre réel, on pourra utilise la forme plus agréable de $g$ afin d'obtenir les antécédents recherchés.
	...
	\exe{Approfondissement}{
		...
		%La somme des premiers impairs est donc toujours un carré parfait.
	}{}
	...
	\str{
		En reprenant l'exercice \ref{ex:5.24}, on peut généraliser la stratégie à employer.
		...
		Pour trouver tous les points d'ordonnée $2$, on peut s'aider en créant une droite horizontale d'ordonnée $2$, comme ci-dessous.
			...
			\begin{tikzpicture}[>=stealth]
				\begin{axis}[xmin = -3.4, xmax=2.3, ymin=-5.1, ymax=5.1, axis x line=middle, axis y line=middle, axis line style=->, grid=both]
					\addplot[no marks, blue, -] expression[domain=-4:3, samples=50]{x^3 /3 - 2*x +3}
					node[pos=.55, above=5pt]{$\mathcal{C}_f$};
					
					\addplot[no marks, red, thick, -] expression[domain=-5:3, samples=50]{2}
					...
					\draw[violet, dotted, thick] (axis cs:-2.6691, 0) -- (axis cs:-2.6691, 2);
					\draw[violet, dotted, thick] (axis cs:0.52398, 0) -- (axis cs:0.52398, 2);
					\draw[violet, dotted, thick] (axis cs:2.1451, 0) -- (axis cs:2.1451, 2);
					
					\addplot[violet] (-2.6691,0) node[below=15pt] {$-2,67$};					
					\addplot[violet] (0.52398,0) node[below=10pt] {$0,52$};					
					\addplot[violet] (2.07,0) node[below=15pt] {$2.14$};
				\end{axis}
			\end{tikzpicture}
			\end{center}
	...
	\str{
		...
		Pour trouver tous les points d'ordonnée inférieure à $2$, on peut s'aider en créant une droite horizontale d'ordonnée $2$, comme ci-dessous.
			\begin{center}
			\begin{tikzpicture}[>=stealth]
				\begin{axis}[xmin = -3.4, xmax=2.3, ymin=-4, ymax=5.1, axis x line=middle, axis y line=middle, axis line style=->, grid=both]
					\addplot[no marks, blue, -] expression[domain=-4:3, samples=50]{x^3 /3 - 2*x +3}
					node[pos=.55, above=5pt]{$\mathcal{C}_f$};
					
					\addplot[no marks, red, thick, -] expression[domain=-5:3, samples=2]{2}
					node[pos=.45, above]{$y=2$};
					...
					\addplot[no marks, violet, thick, -] expression[domain=-3.4:-2.67, samples=2]{0};
					\addplot[no marks, violet, thick, -] expression[domain=.52:2.14, samples=2]{0};
				\end{axis}
			\end{tikzpicture}
			\end{center}
	...
	\thm{}{
		Soit $f:\D \rightarrow \R$ une fonction réelle définie sur son domaine $\D$.
		Pour tout réel $k$, les solutions dans $\D$ de l'équation $f(x) \leq k$ sont les abscisses des points de $\C_f$ siturés en dessous de la droite d'équation $y=k$.
	}{}
		...
		Soient $f,g : ]{-}3,4 ; 2,3[ \rightarrow \R$ deux fonctions réelles sur le domaine $]{-}3,4 ; 2,3[$ dont les courbes représentatives sont données ci-dessous.
			...
			\begin{tikzpicture}[>=stealth]
				\begin{axis}[xmin = -3.4, xmax=2.3, ymin=-4, ymax=8.1, axis x line=middle, axis y line=middle, axis line style=->, grid=both]
					...
					\addplot[no marks, red, -] expression[domain=-4:3, samples=50]{-4*x^2 + 7}
					...
					\draw[violet, dotted, thick] (axis cs:-.8, 0) -- (axis cs:-.8, 4.43);
					\draw[violet, dotted, thick] (axis cs:1.2, 0) -- (axis cs:1.2, 1.176);
					
					\addplot[violet] (-.8,0) node[below=10pt] {$-0,8$};					
					\addplot[violet] (1.2,0) node[below=10pt] {$1,2$};
					
					
					\addplot[violet, mark=|, mark size = 1] (-1.4, 0);
					\addplot[violet] (-1.55, 0) node[above] {$x$};
					\draw[violet, dotted, thick] (axis cs:-1.4, -.84) -- (axis cs:-1.4, 4.8853);
				\end{axis}
			\end{tikzpicture}
			\end{center}
	...
	\ex{}{
		...
		\begin{tikzpicture}[>=stealth]
			\begin{axis}[xmin = -3.4, xmax=2.3, ymin=-4, ymax=8.1, axis x line=middle, axis y line=middle, axis line style=->, grid=both]
				...
				\addplot[violet, mark=|, mark size = 1] (.4, 0) node[below] {$x$};
				\draw[violet, dotted, thick] (axis cs:.4, 0) -- (axis cs:.4, 6.36);
				
				
				\addplot[no marks, violet, thick, -] expression[domain=-.8:1.2, samples=2]{0};
			\end{axis}
		\end{tikzpicture}
		\end{center}
