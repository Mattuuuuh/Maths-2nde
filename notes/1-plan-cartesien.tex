...
\subsection{Distance, longueur, et norme}

\notations{
	Pour deux points $A$ et $B$ du plan, on note $[AB]$ le \emphindex{segment} entre $A$ et $B$, et $AB$ la \emphindex{longueur} du segment.
}

\nt{
	La \emphindex{longueur mesurée} d'un segment n'est pas la même si le repère choisi n'est pas orthonormé.
	...
	En pratique, on choisit rarement des pas identiques valant 1 unité : on ne peut alors pas mesurer la longueur d'un segment à l'aide d'une règle !
}
...
\nomen{
	On parle de \emphindex{distance} entre les points $A$ et $B$ pour la longueur $AB$.
}
