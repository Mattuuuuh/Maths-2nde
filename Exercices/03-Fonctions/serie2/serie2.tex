...
\exe{, difficulty=2}{
	...
	\begin{enumerate}[label=(\alph*)]
		...
		On calclue $f(-2) = 4$ et $h(-2) = 4$, et on en déduit que $x=-2$ exactement.
...
\exemulticols{, difficulty=1}{
	...
	\begin{enumerate}
		...
		On lit (approximativement) les abscisses lorsque la courbe $\C_g$ est sous $\C_h$ :
			\[ \bigset{ x \in \D \tq g(x) \leq h(x) } \approx [ {-0,8} ; {1,2} ]. \]
		\item On cherche ici l'ensemble
			\[ \bigset{ x \in \D \tq h(x) \leq g(x) }. \]
		On lit (approximativement) les abscisses lorsque la courbe $\C_h$ est sous $\C_g$.
