...
\exe{}{
	...
	\begin{enumerate}[label=\roman*)]
		...
		D'où
			\[ \left(\dfrac27 ; -\dfrac17\right) \in \C_f. \]
	\end{enumerate}
}
...
\exemulticols{, difficulty=1}{
	...
	\begin{enumerate}
		...
		On lit (approximativement) les abscisses des points de l'intersection des deux courbes :
			\[ \bigset{ x \in \D \tq g(x) = h(x) } \approx \bigset{ {-0,8}, {1,2} } \]
		\item On cherche ici l'ensemble
			\[ \bigset{ x \in \D \tq g(x) \leq h(x) }. \]
		On lit (approximativement) les abscisses pour lesquelles la courbe $\C_g$ est sous $\C_h$ :
			\[ \bigset{ x \in \D \tq g(x) \leq h(x) } \approx [ {-0,8} ; {1,2} ]. \]
		\item On cherche ici l'ensemble
			\[ \bigset{ x \in \D \tq h(x) \leq g(x) }. \]
		On lit (approximativement) les abscisses pour lesquelles la courbe $\C_h$ est sous $\C_g$.
	...
	Ces points sont exactement les abscisses où une courbe passe au-dessus d'une autre.
}
