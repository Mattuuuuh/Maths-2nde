...
\exe{}{
	Résoudre les équations suivantes pour $x\in\R$ non nul. Exprimer $x$ sous la forme $q \sqrt{n}$ avec $q\in\Q$ rationnel et $n\in\N$ le plus petit possible.
	\begin{multicols}{4}
	\begin{enumerate}
...
\exemulticols{, difficulty=1}{
	Calculer la longueur \textbf{exacte} de chaque côté des deux triangles rectangles ci-contreà l'aide des valeurs exactes données dans le cours.
}{
	\null\vspace{-30pt}
	\centering
	\begin{tikzpicture}[scale=.8]
...
\exemulticols{, difficulty=2}{
	...
	\begin{enumerate}
		...
		\end{tikzpicture}
		\end{center}
		On lit $x^\star \approx 45^\circ$ (c'est en fait une égalité) et $f(x^\star) = 1$.
...
\exemulticols{, difficulty=1}{
	...
	Il existe plusieurs chemins (une infinité), leur point commun étant que la partie dans l'eau est orthogonale aux rives.
	En effet et d'après le cours, lorsque l'homme quitte la rive de départ, le point le plus proche sur l'autre rive est son projeté orthogonal.
...
\exemulticols{, difficulty=1}{
	...
	L'aire du triangle $ABC$ est donc égale à
		\[ \frac12 CD\cdot AB = \frac12 \frac25 \frac34 = \frac3{20}. \]
...
\exe{, difficulty=2}{
	...
	Quelle longueur $AD$ doit-on choisir pour que les droites $(AB)$ et $(CD)$ soient perpendiculaires ?
} {exe:perp-to-aire}{
	Pour que les droites soient perpendiculaires, il faut et il suffit que $D$ soit le projeté orthogonal de $C$ sur $(AB)$.
	Ceci est équivalent à dire que la hauteur du triangle $ABC$ de base $AB$ est égale à $CD$.
	À nouveau, ceci est équivalent au calcul de l'aire
		\[ \frac12AB \cdot CD = 8 \iff CD = \frac{16}{4} = 4. \]
	En outre, le triangle $ACD$ est rectangle en $D$ si et seulement si $AC^2 = AD^2 + CD^2$.
	Ceci est donc équivalent à 
		\[ AD^2 = AC^2 - CD^2 = 5^2 - 4^2 = 9, \]
	et donc à $AD = \pm3$.
	Il faut donc choisir $AD = 3$ pour que $(AB)$ et $(CD)$ soient perpendiculaires.
...
\exe{, difficulty=3}{
	...
	La démonstration fonctionne comme suit : chaque construction à la règle et au compas à partir d'un repère orthonormé ajoute la racine carrée d'un nombre déjà constructible jusqu'ici.
