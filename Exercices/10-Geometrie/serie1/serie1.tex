...
\exe{}{
	Calculer $\cos^2(\theta) + \sin^2(\theta)$ pour $\theta = 30^\circ$ et $\theta =60^\circ$ à l'aide de leurs valeurs exactes.
}{exe:c2s2-vérif}{
	TODO
}

\exe{}{
	Vérifier qu'on a bien $\tan(\theta) = \dfrac{\sin(\theta)}{\cos(\theta)}$ pour $\theta = 30^\circ, 60^\circ$ à l'aide de leurs valeurs exactes.
}{exe:tan-vérif}{
	TODO
}
...
\begin{tabular}{|*{13}{c|}}\hline
	...
	\\\hline
	Exercices & 
	\ref{exe:aut2}, \ref{exe:cos36} & \ref{exe:pyth-thalès}, \ref{exe:c2-s2-1}, \ref{exe:opt-area} & \ref{exe:loi-sinus}  &
