...
\begin{tabular}{|*{13}{c|}}\hline
	...
	\ref{exe:conjugue} & \ref{exe:frac-continue-sqrt2} & \ref{exe:temp-egal}, \ref{exe:Qsqrt2}
	\\\hline
\end{tabular}
\end{center}
...
\exe{}{
	Calculer $\cos^2(\theta) + \sin^2(\theta)$ pour $\theta = 30^\circ$ et $\theta =60^\circ$ à l'aide de leurs valeurs exactes données dans le cours.
}{exe:c2s2-vérif}{
...
\exe{}{
	Vérifier qu'on a bien $\tan(\theta) = \dfrac{\sin(\theta)}{\cos(\theta)}$ pour $\theta = 30^\circ, 60^\circ$ à l'aide de leurs valeurs exactes données dans le cours.
}{exe:tan-vérif}{
...
\exemulticols{, difficulty=0}{
	Calculer la longueur \textbf{exacte} de chaque côté des deux triangles rectangles ci-contre.
}{
	\null\vspace{-30pt}
	\centering
	\begin{tikzpicture}[scale=.8]
...
\begin{multicols}{2}
\begin{Exercise}[difficulty=1, label=exe:pyth-thalès]
...
\exemulticols{}{
	...
	En supposant qu'il avance à vitesse constante dans l'eau, quel est le chemin qu'il doit prendre ?
	\vfill\null
}{
	\null\vspace{-50pt}
	\begin{center}
...
\begin{multicols}{2}
	...
	\end{tikzpicture}
	\end{center}
\begin{Exercise}[label=exe:proj2]
...
% can't think of a way of making this interesting.
%\exe{, difficulty=1}{
%	Montrer sans calcul qu'un triangle de côtés $2 ; 3,5 ; 4,03$ ne peut pas être rectangle.
%}{exe:non-rec}{
%	todo
%}
