...
\exe{, difficulty=1}{
	...
	\begin{enumerate}
		...
		\item
		Calculer la moyenne des salaires mensuels de l'entreprise en 2026 en euros constants.
		Les 50€ de prime ont-ils compensé l'inflation ?
	\end{enumerate}
...
\exe{}{
	Pour chaque histogramme de la figure \ref{fig:hist}, calculer
		\begin{multicols}{2}
		\begin{enumerate}[label=\roman*)]
	...
	Par exemple, de l'histogramme \ref{fig:1a}, on lit 6 notes de valeur 12,5.
}{exe:7}{
	todo
}
...
\begin{figure}[h!]
  ...
  \begin{tikzpicture}[scale=1]
    \begin{axis}[
      ...
      \addplot+[bar shift=5pt,draw=BLACK, thick, fill=BLUE_E!50] plot coordinates {
        (0,1)
        (3,1)
        (5,1)
        (6,2)
  ...
  \begin{tikzpicture}[scale=1]
    \begin{axis}[
      ...
      \addplot+[bar shift=5pt,draw=BLACK,thick, fill=GREEN_E!50] plot coordinates {
        (0,1)
        (1,2)
        (2,2)
        (3,1)
    ...
    \end{axis} 
  \end{tikzpicture}
  \caption{Évaluation ``Fonction carré''.}
  \label{fig:1b}
  \end{subfigure}
  ...
  \\
  \underline{Lecture} : 5 notes de l'évaluation ``Fonction carré'' appartiennent à l'intervalle $[10;11[$.
  }
  \label{fig:hist}
\end{figure}
