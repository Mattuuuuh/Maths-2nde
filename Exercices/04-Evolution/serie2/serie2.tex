...
\exe{}{
	Donner le coefficient multiplicateur associé à 3 diminutions successives de 10\% chacune.
	
	Comparer l'évolution totale avec une diminution simple de 30\%.
}{exe:c1}{
	Une diminution de 10\% est associée au coefficient multiplicateur $1-\frac{10}{100} = 0,9$.
	Appliqué 3 fois, il devient $0,9 \times 0,9 \times 0,9 = 0,9^{3} = 0,729$.
	Comme $0,729 -1 = -0,271 = -27,1\%$, ceci correspond à une diminution de 27,1\%.
	
	En conclusion, effectuer trois diminutions de 10\% aboutit à une diminution plus petite qu'une unique diminution de 30\%.
	
	En général, on ne compare jamais $n$ évolutions de $P\%$ avec une évolution de $nP\%$.
	Cette comparaison n'a d'ailleurs pas de sens lorsque $nP$ dépasse 100 lors d'une diminution.
}

\exe{}{
	Donner le coefficient multiplicateur associé à 20 augmentations successives de 5\% chacune.
	
	Comparer l'évolution totale avec une augmentation simple de 100\%.
}{exe:c2}{
	Une augmentation de 5\% est associée au coefficient multiplicateur $1+\frac{5}{100} = 1,05$.
	Appliqué 20 fois, il devient $1,05 \times \cdots \times 1,05 = 1,05^{20} \approx 2,65$.
	Comme $2,65 -1 = 1,65 = +165\%$, ceci correspond à une augmentation de 165\%.
	
	En conclusion, effectuer trois augmentation de 10\% aboutit à une augmentation plus grande qu'une unique augmentation de 100\%.
...
\exe{}{
	Une colonie de 10 puces s'installe sur le dos d'un chat. Chaque jour, la taille de la colonie augmente de 2\%.
	Estimer la taille de la colonie après 1 an (365 jours) sans traitement anti-parasitaire.
}{exe:5}{
	Une augmentation de 2\% correspond à multiplier par $1+\frac{2}{100} = 1,02$.
	Après 365 telles augmentations, on estime la population de puces en calculant
		\[ 10 \times 1,02^{365} \approx 13~774. \]
}
...
\exe{}{
	...
	\begin{enumerate}[label=\roman*)]
		...
		\item $7^{-12}$
		\item $10^{24}$
		\item $5^{-8-6} = 5^{-14}$ ou alors $25^{-4 -3} = 25^{-7}$.
	\end{enumerate}
	\end{multicols}
}
...
\exe{, difficulty=1}{
	...
	Combien de grains de riz ont été produits en 2022 ? Répondre en écriture scientifique.
...
\exe{, difficulty=2}{
	Combien d'opérations arithmétiques élémentaires (additions ou multiplications) sont nécessaires \emph{a priori} pour calculer $17^{32}$ ?
