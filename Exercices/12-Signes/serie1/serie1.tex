...
\exemulticols{}{
	...
	\begin{axis}[xmin = -3, xmax=3, ymin=-2.5, ymax=3.5, y=25pt, axis x line=middle, axis y line=middle, axis line style=->, xlabel={}, ylabel={}, xtick = {-4, -3, ..., 4}, ytick = {-4, -3, ..., 4}, grid=both]
		
		% (g)
		\addplot[RED_E, very thick, domain =-3:3, samples=50] {(x+2.5)*x*(x-3)/5}  node[right] {$\C_g$};
		
		% (f)
		\addplot[BLUE_E, very thick, domain =-3:3, samples=50] {6*exp(-x^2/5)-2.7}  node[right] {$\C_f$};
	\end{axis}
	\end{tikzpicture}
	\end{center}
}{exe:aut0}{
	...
	\begin{enumerate}
		\item On a environ 
			\[ \{ x \in [-3 ; 3] \text{ tq. } f(x) \geq 0\} \approx [-2 ; 2]. \]
		\item On a environ 
			\[ \{ x \in [-3 ; 3] \text{ tq. } g(x) \leq 0\} \approx [-3 ; -2,5] \cup [0 ; 3]. \]
	\end{enumerate}
}
...
\exe{}{
	Remplir approximativement les tableaux ci-dessous à l'aide des graphes de $f$ et $g$ sur le domaine $\D = [-3;  3]$.
	

	\begin{center}
	\hspace{110pt}
	\begin{tikzpicture}[>=stealth, scale=1]
	\begin{axis}[xmin = -3, xmax=3, ymin=-2.5, ymax=3.5, y=25pt, axis x line=middle, axis y line=middle, axis line style=->, xlabel={}, ylabel={}, xtick = {-4, -3, ..., 4}, ytick = {-4, -3, ..., 4}, grid=both, x=55pt]
	...
	\begin{tikzpicture}
		\tkzTabInit
		 [lgt=4,espcl=2]
	       		{$x$ / 1 , Signe de $f(x)$ / 1, Signe de $g(x)$ / 1, Signe du produit $f(x)\cdot g(x)$ / 1}
	       		{,,,,,,}
	\end{tikzpicture}
	\end{center}
	
	On en déduit que $\bigset{ x \in [-3 ; 3] \text{ tq. } f(x) \cdot g(x) \geq 0} = $
	...
	On en déduit que $\bigset{ x \in [-3 ; 3] \text{ tq. } f(x) \cdot g(x) \geq 0} = [-3;-2,5]\cup[-2;0]\cup[2;3]$.
}
...
\begin{Exercise}[label=exe:10]
	Esquisser la droite $y=3x - 2$ et remplir le tableau de signes ci-dessous.
...
\begin{Exercise}[label=exe:11]
	Esquisser la droite $y = -6x - 2$ et remplir le tableau de signes ci-dessous.
...
\exe{}{
	Pour chaque fonction affine $f$ suivante, calculer l'intervalle 
		$ \bigset{ x \in \R \text{ tq. } f(x) \geq 0 } $ et faire un dessin rapide de $\C_f$.
	...
	\begin{enumerate}
		...
		On obtient donc 
			\[ \{ x \in \R \text{ tq. } x+ 4 \geq 0 \} = \{ x \in \R \text{ tq. }x \geq -4 \} = [-4 ; \pinfty[. \]
		\item 
			\[ \left[ \dfrac{14}3 ; \pinfty \right[ \]
		\item
			\[ [0 ; \pinfty[ \]
		\item
		L'inégalité $-4x - 16 \geq 0$ est équivalente à $-4x \geq 16$.
		Pour continuer, on multiplie par $-\dfrac14$ qui est négatif, ce qui change donc le sens de l'inégalité : $x \leq -4$.
		On obtient donc
			\[ ]\minfty ; -4]. \]
		\item
			\[ ]\minfty ; 0] \]
...
\exe{}{
	...
	Remplir les tableaux de signes ci-dessous puis décrire les ensembles suivants.
		\begin{align*}
			\bigset{ x \in \R \text{ tq. } f(x) \geq 0 } &= && && &&
			\\ 
			\bigset{ x \in \R \text{ tq. } g(x) \leq 0 } &= && && &&
		\end{align*}
	...
	\begin{tikzpicture}
		\tkzTabInit
		 [lgt=4.5,espcl=4]
	       		{$x$ / 1 , Signe de $x+3$ / 1, Signe de $3x-10$ / 1 , Signe de \mbox{$f(x) = (x+3)(3x-10)$} / 1}
	       		{,,,}
	...
	\begin{tikzpicture}
		\tkzTabInit
		 [lgt=4.5,espcl=4]
	       		{$x$ / 1 , Signe de $x$ / 1, Signe de $-4x-12$ / 1 , Signe de \mbox{$g(x) = x(-4x-12)$} / 1}
	       		{,,,}
...
%		\tkzTabInit
%		 [lgt=5.5,espcl=3]
%	       		{$x$ / 1 , Signe de $x+3$ / 1, Signe de $-10x$ / 1 , Signe de $3x-10$ / 1 , Signe de \mbox{$h(x) = -10x(x+3)(3x-10)$} / 1}
%	       		{,, , ,}
%	   
%	\end{tikzpicture}
	\end{center}
...
%	\end{tikzpicture}
	...
	Par exemple, $x+3$ s'annule en $x=-3$, est positif après $-3$, et est négatif avant.
	...
	Le deuxième tableau est similaire, en faisant attention au fait que le signe de $-4x-12$ est positif puis négatif.
...
\exemulticols{}{
	...
	\begin{center}
	\begin{tikzpicture}[>=stealth, scale=1]
	\begin{axis}[xmin = -3, xmax=3, ymin=-2.5, ymax=3.5, axis x line=middle, axis y line=middle, axis line style=->, xlabel={}, ylabel={}, xtick = {-4, -3, ..., 4}, ytick = {-4, -3, ..., 4}, grid=both, x=40pt, y=20pt]
		% (h)
		\addplot[BLUE_E, very thick, domain =-3:3, samples=50] {6*exp(-x^2/5)-2.7}  node[right] {$\C_h$};
	\end{axis}
	\end{tikzpicture}
	\end{center}
...
\exe{}{
	...
	\begin{enumerate}
		...
		\item En déduire le domaine de définition $\D_f$.
		\item Remplir le tableau de signes ci-dessous.
		\item Décrire $\bigset{ x \in \D_f \text{ tq. } f(x) \geq 0 } = $
	\end{enumerate}
	...
	\begin{tikzpicture}
		\tkzTabInit
		 [lgt=3,espcl=3.5]
	       		{$x$ / 1, Signe de $x-3$ / 1 , Signe de $7x+1$ / 1, Signe de $2x+1$ / 1 , Signe de $f(x)$ / 1}
	       		{,, , ,}
	...
	\begin{enumerate}
		...
		Le dénominateur $(x-3)(7x+1)$ est nul lorsqu'un des deux facteur est nul, c'est-à-dire lorsque $x=3$ ou $x= -\frac17$.
		Par conséquent, 
			\[ \D_f = \R - \left\{ 3 ; -\dfrac17 \right\}. \]
		...
		\item 
			\[ \bigset{ x \in \D_f \text{ tq. } f(x) \geq 0 } = \left[ -\dfrac12 ; -\dfrac17 \right[ \cup ]3 ; \pinfty[. \]
...
\exe{, difficulty=2}{
	...
	\end{enumerate}
	Le but des questions suivantes est d'effectuer la division euclidien de $3x^2-7x+4$ par $x-1$ afin de trouver la deuxième racine de l'équation.
	\begin{enumerate}[resume]
		\item
...
% peut-être donner un polynôme explicite fonction de r, s, d peut être mieux ?
%\exe{, difficulty=2}{
%	Montrer que n'importe quel nombre réel de la forme $r+s\sqrt{d}$, avec $r, s\in\Q$ et $d\in\N$, est zéro d'un polynôme quadratique à coefficients entiers $f(x) = ax^2 + bx + c$, avec $a, b, c \in \Z$.
%}{exe:all-roots}{
%	etc...
%}
...
\exe{, difficulty=3}{
	...
	Voici une stratégie de preuve possible.
	\begin{enumerate}
		\item
		Montrer que $1$ et $\sqrt{d}$ sont \emph{linéairement indépendants} sur $\Q$.
		C'est-à-dire, montrer que si $u + v\sqrt{d} = 0$ avec $u, v \in\Q$, alors $r=s=0$.
