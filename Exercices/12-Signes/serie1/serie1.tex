...
\exemulticols{}{
	...
	\begin{axis}[xmin = -3, xmax=3, ymin=-2.5, ymax=3.5, y=25pt, axis x line=middle, axis y line=middle, axis line style=->, xlabel={}, ylabel={}, xtick = {-4, -3, ..., 4}, ytick = {-4, -3, ..., 4}, grid=both]
		...
		% (g)
		\addplot[RED_E, very thick, domain =-3:3, samples=50] {(x+2.5)*x*(x-3)/5+.5}  node[right] {$\C_g$};
		
		% (f)
		\addplot[BLUE_E, very thick, domain =-3:3, samples=50] {6*exp(-x^2/5)-3}  node[right] {$\C_f$};
	\end{axis}
	\end{tikzpicture}
	\end{center}
}{exe:aut0}{
...
\exe{}{
	...
	\begin{axis}[xmin = -3, xmax=3, ymin=-2.5, ymax=3.5, y=25pt, axis x line=middle, axis y line=middle, axis line style=->, xlabel={}, ylabel={}, xtick = {-4, -3, ..., 4}, ytick = {-4, -3, ..., 4}, grid=both, x=55pt]
		...
		% (g)
		\addplot[RED_E, very thick, domain =-3:3, samples=50] {(x+2.5)*x*(x-3)/5+.5}  node[pos=0,left] {$\C_g$};
		
		% (f)
		\addplot[BLUE_E, very thick, domain =-3:3, samples=50] {6*exp(-x^2/5)-3}  node[pos=0,left] {$\C_f$};
	\end{axis}
	\end{tikzpicture}
	...
	$\bigset{ x \in [-3 ; 3] \text{ tq. } f(x) \cdot g(x) \geq 0} = $
...
\exe{}{
	...
	Remplir les tableaux de signes ci-dessous et en déduire
		\begin{align*}
			%\bigset{ x \in \R \text{ tq. } f(x) \geq 0 }, && \bigset{ x \in \R \text{ tq. } g(x) \leq 0 }, && \bigset{ x \in \R \text{ tq. } h(x) \geq 0 }
			\bigset{ x \in \R \text{ tq. } f(x) \geq 0 }, && \et && \bigset{ x \in \R \text{ tq. } g(x) \leq 0 }
		\end{align*}
...
\exe{}{
	...
	\begin{enumerate}
		...
		\item Exprimer $\bigset{ x \in \D_f \text{ tq. } f(x) \geq 0 }$ sous forme d'union d'intervalles.
	\end{enumerate}
...
\exe{, difficulty=2}{
	...
	\begin{enumerate}
		...
		\item
		Quelle fonction $q_1(x)$ prendre pour que $3x^2 -7x+4 - (x-1)q_1(x)$ n'ait plus de terme en $x^2$ ?
		\item
		Quelle fonction $q_2(x)$ prendre pour que $3x^2 -7x+4 - (x-1)q_1(x) - (x-1)q_2(x)$ n'ait plus de terme en $x$ ?
		\item
