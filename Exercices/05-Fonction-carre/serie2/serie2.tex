...
\exe{,difficulty=1}{
	...
	\begin{enumerate}[label=\alph*), leftmargin=50pt]
		\item Quels $x$ vérifient $f(x) = -3$ ?
		\item Quels $x$ vérifient $f(x) = -4$ ?
		\item Quels $x$ vérifient $f(x) \leq 6$ ?
		\item Quels $x$ vérifient $f(x) = 10$ ?
	...
	\begin{enumerate}[label=\alph*)]
		\item
		Comme l'unique minimum de $f$ est $-3$, seul $x^\star = -\frac12$ vérifie $f(x) = -3$.
			\[ \bigset{ x \in\R \tq f(x) = -3 } = \Bigset{ - \dfrac12 }. \]
		\item
		Comme le minimum de $f$ est $-3$, aucun $x \in\R$ ne peut vérifier $f(x) < -3$.
			\[ \bigset{ x \in\R \tq f(x) = -4 } = \emptyset \text{ (l'ensemble vide). } \]
		\item 
		...
		%Pour poursuivre, il convient d'utiliser la propriété $|E| \leq a \iff -a \leq E \leq a$.
		Pour poursuivre, il convient d'utiliser la propriété $|E| = a \iff E = a$ ou $E=-a$.
%			\begin{alignat*}{2}
...
%			\end{alignat*}
		Il suit donc que
			...
			\[ \bigset{ x \in\R \tq f(x) = 10 } = \Bigset{\dfrac{-\sqrt{13}-1}2 ; \dfrac{\sqrt{13}-1}2 } \]
		\item 
		...
		Pour poursuivre, il convient d'utiliser la propriété $|E| \leq a \iff -a \leq E \leq a$.
			...
			\end{alignat*}
		Il suit donc que
			\[ \bigset{ x \in\R \tq f(x) = 22 } = \left[-3 ; 2\right] \]
			%\[ \bigset{ x \in\R \tq f(x) = 22 } = \bigset{-3 ; 2} \]
		\item 
		Cet ensemble est le complémentaire à $\R$ du dernier ensemble, car on a soit $f(x) \leq 22$, soit $f(x) > 22$.
		Par conséquent,
			\[ \bigset{ x \in\R \tq f(x) = 22 } = \left[ \minfty ; -3\right[ \cup \left]2 ; \pinfty\right] \]
	\end{enumerate}
}
...
\exe{, difficulty=1}{
	Soient $x, y\geq0$. Montrer que $\sqrt{x}-\sqrt{y} = \dfrac{x-y}{\sqrt{x}+\sqrt{y}}$ et en déduire que \mbox{$x < y \iff \sqrt{x} < \sqrt{y}$}.
}{exe:sqrt-croissante}{
