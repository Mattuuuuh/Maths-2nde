...
\exe{, difficulty=1}{
	...
	\begin{enumerate}
		...
		\item
		En déduire une valeur $x\in\N$ solution de l'équation originale.
		\item
		Vérifier la solution en substituant $x$ par la valeur trouvée.
	\end{enumerate}
}{exe:div105}{
	todo
}
...
\exe{, difficulty=2}{
	...
	Indication pour la question 4 : soustraire $n+1$ des deux côtés de l'équation obtenue à la question 3.
	
	En considérant les différences successives de cubes, on peut montrer que
		\[ 1 + 4 + 9 + \cdots + n^2 = \dfrac{n(n+1)(2n+1)}6. \]
	On obtient d'ailleurs que $n(n+1)(2n+1)$ est toujours un multiple de 6 (résultat qu'on peut démontrer de façon plus simple).
}
