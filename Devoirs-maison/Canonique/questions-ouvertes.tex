...
\exe{, difficulty=2}{
	...
	Comme $f$ s'évanouit vers l'infini, $f(x)$ est toujours plus petite que $f(-1)$ et toujours plus grande que $f(-1)$ pour $x$ suffisamment grand (positif ou négatif).
...
\exe{, difficulty=3}{
	...
	Pour le point 3 : voir $\frac{f(x-1) + f(x+1)}{2}$ comme une moyenne.
}
