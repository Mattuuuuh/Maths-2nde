...
%% THEN RUN THIS
\input{preamble.tex}\input{adr/vars_BASELINE.adr}
...
\exe{
	 Démontrer que l'aire $\mathcal{A}(x)$ du triangle $OAB$ est donnée par, pour $x\in[0;\xmax]$,
		\[ \mathcal{A}(x) = \prodovertwo \cdot |10-x| \cdot |x| =  \prodovertwo(\xmax-x)x. \]
}{}
\exe{
...
\exe{
	 Montrer que, pour tout $x\in[0;\xmax]$, on a l'identité
		\[ \mathcal{A}(x) =  \BETA - \minusA\left(x-\ALPHA\right)^2. \]
}{}
\exe{
