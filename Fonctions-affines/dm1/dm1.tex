...
%% THEN RUN THIS
\input{preamble.tex}\input{adr/vars_44284.adr}
...
\exe{\label{ex:2}
	Considérons une fonction linéaire $f(x) = 0,16x$.
	...
	\begin{enumerate}
		...
		Quelle est l'ordonnée maximale d'un point $C$ d'abscisse $13$ si la pente de la fonction linéaire passant par $C$ ne peut pas dépasser $10\%$ ?
		\item
		Calculer l'angle $\beta = \widehat{COB}$ dans ce cas.
		\item 
		À quel angle $\gamma$ correspond une pente de $100\%$ ?
		\item 
		À quel pourcentage de pente correspond un angle de $20$° ?
	\end{enumerate}
