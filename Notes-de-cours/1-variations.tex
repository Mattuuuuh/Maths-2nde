...
\exe{}{
	Expliquer avec des mots pourquoi la fonction $\cos$ est décroissante et la fonction $\sin$ est croissante sur $[0\degree ; 90\degree]$
...
\exe{}{
	Esquisser la courbe représentative de la fonction affine $f(x) =3x - 2$ sur le domaine $\D = [-10 ; 12]$ puis remplir le tableau de variations et de signes ci-dessous.
...
\exe{}{
	Esquisser la courbe représentative de la fonction affine $f(x) = -6x - 2$ sur le domaine $\D = [-10 ; 8]$ puis remplir le tableau de variations et de signes ci-dessous.
...
\exe{}{
	Esquisser la courbe représentative de la fonction affine $f(x) = -3$ sur le domaine $\D = [-12 ; -5]$ puis remplir le tableau de variations et de signes ci-dessous.
...
\notations{
	On pose $\R_-^* = ]\minfty ; 0[$ les nombres réels strictement négatifs, et $\R_+^* = ]0 ; \pinfty[$ les nombres réels strictement positifs.
}
...
\exe{, difficulty=1}{
	Montrer que si $0\leq x<y$, alors 
		\[ \sqrt{y} - \sqrt{x} = \dfrac{y - x}{\sqrt{y} + \sqrt{x}} > 0. \]
	En déduire le théorème \ref{thm:var-sqrt}.
}{exe:var-sqrt}{
	Soient $0\leq x<y$. On souhaite montrer que $\sqrt{x} < \sqrt{y}$, c'est-à-dire que $\sqrt{y} - \sqrt{x} > 0$.
	
	Or, comme $\sqrt{x}+\sqrt{y}>0$ est non nul, on peut écrire 
		\[ \sqrt{y} - \sqrt{x} = \dfrac{\bigl(\sqrt{y} - \sqrt{x}\bigr)\bigl(\sqrt{y} + \sqrt{x}\bigr)}{\sqrt{y} + \sqrt{x}} 
						= \dfrac{y - x}{\sqrt{y} + \sqrt{x}}. \]
	Le numérateur étant positif car $x<y$ et le dénominateur aussi, on conclut comme requis.
}
...
\ex{forme canonique : du signe du carré à l'extremum}{
	...
	Multiplier par $-1$ change le sens de l'inégalité et on obtient, en ajoutant $\dfrac{265}2$,
		\begin{align*}
