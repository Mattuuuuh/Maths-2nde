...
\nt{
	...
	Lorsqu'on trace un repère, le pas en abscisse n'est pas forcément le même que le pas en ordonnée : pour représenter les points $(-1 ; 35), (2 ; 70),$ et $(-2 ; -40)$, mieux vaut choisir un grand pas en ordonnée, et un petit pas en abscisse.
	
	\textbf{\warning Il est important que le pas soit toujours régulier.}
	
	\begin{multicols}{2}
	\begin{center}
	\includegraphics[page=11,scale=.8]{figures/fig-plan.pdf}
	\end{center}
	
	En outre, il est toujours possible de ne représenter qu'une partie d'un repère.
	
	Si les points à représenter se situent tous après $x=4$ et avant $y=-10$, il n'est pas utile de représenter l'origine $O$.
	Cependant, il est toujours utile de représenter une graduation en abscisse et en ordonnée pour le placement et la lecteur des coordonées des points.
	
	Un exemple est donné ci-contre. Les pas en abscisse et en ordonnée sont tous les deux 1.
...
\notations{
	...
	C'est aussi la longueur du \emphindex{segment} $[AB]$.
}
...
\nt{
	Le carré d'un nombre est toujours positif : $x^2 \geq 0$ pour tout $x\in\R$.
}
...
\exe{}{
	Donner le carré de la norme de $(0 ; 0)$, $(3 ; 4)$, $(-3 ; 4)$, $(-1; -1)$.
}{exe:norme}{
	\begin{align*}
...
\nt{
	La manipulation des points est considérée illégale par une partie des mathématiciens.
	Pour ne pas les vexer, remplacer tous les points $A(x ;y)$ par un vecteur $a = \pvec{x}{y}$ dans la suite.
...
\exe{, difficulty=2}{
	...
	\begin{multicols}{2}
	\begin{center}
	\includegraphics[page=12]{figures/fig-plan.pdf}
	\end{center}
...
\exe{, difficulty=2}{
	Soient $A, B, C$ trois points distincts du plan formant un triangle.
	Posons
		\[ G = \dfrac{A+B+C}3, \]
	son \emphindex{barycentre}, aussi appelé \emphindex{centre de gravité}.
	
	Montrer que $G$ appartient à la médiane du triangle issue de $A$, c'est-à-dire au segment $[AM]$, où $M$ est le milieu du segment $[BC]$.
	
	Montrer que $G$ est le point d'intersection des trois médianes du triangle.
}{exe:mediane-barycentre}{
	On souhaite montrer que $G \in [AM]$, où $M = \frac{B+C}2$.
	Par définition, il faut que $G$ s'exprime comme $\lambda A + (1-\lambda)M$ pour un certain $0\leq\lambda\leq1$.
	
	Or,
		\[ \lambda A + (1-\lambda)M = \lambda A + \dfrac{1-\lambda}2B + \dfrac{1-\lambda}2C. \]
	Pour obtenir $G = \frac13 A + \frac13 B + \frac13 C$, il est naturel de tester $\lambda=\frac13$, qui fonctionne, car
		\[ \dfrac13 A + \dfrac{1-\frac13}2B + \dfrac{1-\frac13}2C = G. \]
	On a donc obtenu légèrement plus fort grâce au poids $\lambda=\frac13$ : le barycentre $G$ est exactement deux fois plus proche de $M$ que de $A$.
	
	Finalement, tout le problème est entièrement symétrique par permutation des symboles $A, B, C$.
	Il suit que $G$ appartient également à la médiane du triangle issue de $B$, et de $C$, ce qui conclut.
}
	
\exe{, difficulty=3}{
	Considérons un nombre $P$ se décomposant en produit d'entiers de deux manières différentes :
		\[ P = a \times b = x \times y. \]
	\begin{enumerate}
		\item
		Montrer que 
			\begin{align*}
				P &= \Bigl( \dfrac{a+b}2 + \dfrac{a-b}2\Bigr)\Bigl( \dfrac{a+b}2 - \dfrac{a-b}2\Bigr) \\
				&= \Bigl( \dfrac{a+b}2 \Bigr)^2 - \Bigl( \dfrac{a-b}2 \Bigr)^2
			\end{align*}
		\item
		En conclure que $4P = r^2 - s^2$ pour deux entiers $r, s \in \Z$.
		\item
		En procédant similairement, montrer que $4P = v^2 - w^2$ pour deux entiers $v, w \in \Z$, différents de $r$ et $s$ respectivement.
		\item 
		Conclure que le triangle $OAB$, de sommets	
			\begin{align*}
				O(0;0), && A(r ; w), && \et && B(s ; v)
			\end{align*}
		est isocèle.
		\item
		Ce triangle peut-il être équilatéral ?
	\end{enumerate}
}{exe:isocele-gen}{
	\begin{enumerate}
		\item
		On exprime simplement $a$ et $b$ comme étant la valeur moyenne ($\frac{a+b}2$) plus ou moins la moitié de la distance entre $a$ et $b$ ($\frac{b-a}2$).
		
		On utilise ensuite l'identité remarquable $(\alpha+\beta)(\alpha-\beta) = \alpha^2 - \beta^2$.
		\item
		En posant $r =a+b$ et $s=a-b$, l'identité $P = \left(\frac{r}2\right)^2 -  \left(\frac{s}2\right)^2$ est équivalente à $4P = r^2 - s^2$.
		\item
		On répète le processus en remplaçant $a$ et $b$ par $x$ et $y$.
		On a donc $v = x+y$ et $w = x-y$, différents de $r$ et $s$ car il existe un unique ensemble de nombres à somme et produit fixés.
		En effet, en supposant que $P = a\times b = x \times y$ et que $S = a+b = x+y$, alors $a, b, x, y$ vérifient
			\[ (t -a)(t-b) = (t-x)(t-y) \]
		pour n'importe quel $t\in\R$.
		En particulier, ce produit s'annule uniquement en $a$ et $b$ à gauche, et uniquement en $x$ et $y$ à droite, ce qui conclut.
		\item 
		Le triangle est isocèle en $O$ car
			\[ \norm{A}^2 = r^2 + w^2 = s^2 + v^2  = \norm{B}^2 \]
		parce que $r^2 - s^2 = v^2 - w^2$.
		\item
		Question laissée ouverte.
	\end{enumerate}
}
	
% mis au chapitre affine (exe:médiatrice, n°234 actuellement).
%\exe{3, difficulty=3}{
...
%		\[ y = \dfrac12 x + \dfrac34. \]
%}{exe:médiatrice}{
%	On impose $\norm{A-M}^2 = \norm{B-M}^2$ et on développe calmement les expressions suivantes, toutes équivalentes.
%		\begin{align*}	
%			\norm{A-M}^2 &= \norm{B-M}^2 \\
%			(2-x)^2 + (3-y)^2 &= (3-x)^2 + (1-y)^2 \\
%			x^2 - 4x + 4 + y^2 - 6y + 9 &= x^2 - 6x + 9 + y^2 - 2y + 1 \\
%			2x + 3 &= 4y \\
%			y &= \dfrac12 x + \dfrac34 y
%		\end{align*}
%}
