...
\exemulticols{1}{
	On considère les fonctions $f$ et $g$ données graphiquement ci-contre.
	Posons $h(x) = f(x)g(x)$.
	\begin{enumerate}[label=--]
		\item Quel est le signe de $f(1)$ ? 
		\item Et $g(1)$ ?
		\item Quel est signe de $h(1)$ ?
		\item Quel est le signe de $f(-1)$ ? 
		\item Et $g(-1)$ ?
		\item Quel est signe de $h(-1)$ ?
	\end{enumerate}
	...
	On lit $f(1) > 0$ et $g(1) < 0$.
	Il suit que $h(1) < 0$, comme produit d'un positif avec un négatif.
	
	On lit $f(-1) > 0$ et $g(-1) > 0$.
	Il suit que $h(-1) > 0$, comme produit d'un positif avec un positif.
}

\exe{, difficulty=1}{
	Pour $f, g$ les fonctions de l'exercice \ref{exe:signes0}, exprimer par lecture graphique les ensembles suivants sous forme d'union d'intervalles.
		\begin{multicols}{2}
		\begin{enumerate}
			\item $\bigset{ x \in [-3 ; 3] \tq f(x) \geq 0 }$
			\item $\bigset{ x \in [-3 ; 3] \tq f(x) \leq 0 }$
	...
	\begin{enumerate}
		...
		\item On a environ 
			\[ \bigset{ x \in [-3 ; 3] \tq f(x) \leq 0 } \approx [-3; -1,5] \cup [1,5 ; 3]. \]
		Remarquons que c'est le complémentaire de l'ensemble précédent, car $f(x)$ est soit positive ou nulle, soit négative ou nulle.
		\item On a environ 
...
\exe{}{
	Pour quels $x\in\R$ a-t-on $(x-12)(x+3)(x-1) = 0$ ?
}{exe:lecture-racines}{
	TODO
}

\exe{, difficulty=1}{
	Pour quels $x\in\R$ a-t-on $x(x-1)^2 = 0$ ?
}{exe:lecture-racines2}{
	TODO
}
...
\nt{
	...
	Si $a>0$, $f$ est croissante, donc $f(x) \geq 0$ sur $[x_0 ; \pinfty[$.
	Sinon, $a <0$, et $f$ est décroissante, donc $f(x) \geq 0$ sur $]\minfty ; x_0 ]$.
}
...
\exe{, difficulty=2}{
	Considérons $a, b, x, y \in \R$ quatre réels vérifiant
		\begin{align*}
			a+b = x+y, && \et && ab = xy.
		\end{align*}
	Montrer que $(t-a)(t-b) = (t-x)(t-y)$ pour tout $t\in\R$, et conclure que $\{ a ; b \} = \{ x ; y \}$.
}{exe:S-P}{
	TODO
}
