...
\exe{, difficulty=1}{
	Montrer que $x = \left( \frac{x+1}2 \right)^2 - \left(\frac{x-1}2 \right)^2$ pour tout $x\in\R$, et en conclure que tout impair est différence de carrés parfaits.
}{exe:pair-diff-sq}{
	TODO
}
...
\exe{, difficulty=2}{
	...
	La somme est dite \emphindex{somme téléscopique} car chaque terme en plus annule le précédent.
...
\exe{, difficulty=2}{
	...
	car $n+1=\underbrace{1+1+\cdots+1}_\text{$n+1$ fois}$ et car la somme de gauche a $n+1$ termes.
...
\exe{}{
	Montrer que si $0 < n < a$, alors $a$ ne peut pas diviser $n$.
}{exe:mod-a}{
	Si $a$ divise $n$, alors $n$ est un multiple de $a$.
	Or les multiples de $a$ sont $0, a, 2a, \cdots$, et $n$ appartient à cette liste.
	Comme $0 < n < a$, $n$ n'est pas un multiple de $a$.
}

\exe{, difficulty=1}{
	Montrer que si $a > \frac{n}2$ divise $n\neq0$, alors $a=n$.
}{exe:diviseur-moitié}{
	Si $a$ divise $n$, alors $n$ est un multiple de $a$.
	Or les multiples de $a$ sont $0, a, 2a, \cdots$, et $n$ appartient à cette liste.
	Comme $a > \frac{n}2$, on a $2a > n$.
	La seule possibilité est donc $n=a$, car $n\neq0$.
}
...
\exe{, difficulty=2}{
	Montrer que si $a|b$ et $b|a$, alors $a=b$ ou $a=-b$.
}{exe:div-implies-is}{
	Il existe deux entiers $k$ et $\ell$ vérifiant $b = ak$ et $a = b\ell$.
	Il suit que $b = b \ell k$, égalité d'entiers.
	
	Si $b=0$, alors $a = 0$, ce qui conclut.
	Sinon, diviser par $b$ donne $\ell k = 1$.
	Or, le produit de deux entiers ne peut valoir 1 que lorsqu'ils sont tous les deux égaux, à 1 ou -1.
	Ces cas correspondent à $a=b$ et $a=-b$ respectivement.
}
...
\exe{, difficulty=2}{
	Considérons deux entiers tels que leur somme vaut leur produit.
	Montrer qu'ils sont tous les deux égaux à 0 ou tous les deux égaux à 2.
}{exe:sum-is-product}{
	Nommons ces deux entiers $a, b \in \Z$.
	La relation $a+b = ab$ implique que $a = b(a-1)$, et donc que $b$ divise $a$.
	Symétriquement, $a$ divise $b$.
	
	Par conséquent, et d'après l'exercice \ref{exe:div-implies-is} $a=b$ ou $a=-b$, et la relation initiale devient $2a = a^2$ ou $0 = -a^2$. 
	$a=0$ est bien sûr solution des deux équations (et la seule solution de la deuxième, d'ailleurs). 
	Si $a\neq0$, on peut diviser par $a$ pour trouver $a = 2$, l'autre solution.
}
...
\pf{théorème \ref{thm:règle-divisibilité-3-9} pour la divisibilité par 3}{
	...
	En général, $10^p \cdot a$ est divisible par 3 si et seulement si $a$ l'est, ce qui conclut.
}{}
...
\thm{théorème fondamental de l'arithmétique}{
	\emph{La} \emphindex{décomposition en facteurs premiers} est \emphindex{unique} à permutation des facteurs près.
...
\exe{, difficulty=1}{
	...
	Une autre façon de faire est d'utiliser le théorème \ref{thm:divisibilité-combinaison-entière}.
	Comme $140 = 7\times20$ est divisible par 7, alors 144 est divisible par 7 si et seulement si $144-140 = 4$ l'est.
	Il ne l'est bien sûr pas.
}
...
\exe{}{
	Donner tous les diviseurs communs à 24 et 32 puis montrer qu'on peut \emph{réduire} la fraction $\frac{32}{24}$ comme suit :
		\[ \dfrac{32}{24} = \dfrac86. \]
	Peut-on réduire davantage la fraction $\frac86$ ?
}{exe:div-inter}{
	todo
}
...
\dfn{intersection}{	
	Pour deux ensembles $A, B$ on définit $A \cap B$ l'\emphindex{intersection} des deux ensembles.
	Un élément appartient à $A \cap B$ dès qu'il appartient à $A$ \textbf{et} à $B$.
}{}

\dfn{nombres premiers entre eux}{
	Soit $a, b \in \N$ entiers supérieurs à 1.
	Les quatre propositions suivantes sont équivalentes.
	\begin{enumerate}[label=\roman*)]
		\item $a$ et $b$ sont \emphindex{premiers entre eux}.
		\item Le seul diviseur commun à $a$ et $b$ est 1.
		\item $\D_a \cap \D_b = \{ 1 \}$.
		\item Les nombres $\frac{a}{b}$ et $\frac{b}{a}$ sont des \emphindex{fractions irréductibles}.
	\end{enumerate}
}{dfn:coprimalité}


\exe{}{
	Donner les diviseurs \emph{communs} à $6$ et à $15$.
	Simplifier la fraction $\frac{6}{15}$ en \emph{fraction irréductible}.
}{exe:old-div4}{
	$\mathcal{D}_6 \cap \mathcal{D}_{15} = \{ 1, 3 \}$.
	
	\[
		\dfrac{6}{15} = \dfrac{3 \times 2}{3 \times 5} = \dfrac25,
	\]
	irréductible car $2$ et $5$ sont premiers entre eux. 
	
	En fait, lorsqu'on simplifie par le \emphindex{plus grand commun diviseur} (pgcd), la fraction devient nécessairement irréductible.
}


\exe{}{
	Donner les multiples \emph{communs}  à $10$ et $35$ inférieurs ou égaux à $100$.
	Écrire la somme $\frac{3}{10} + \frac{6}{35}$ en \emph{fraction irréductible}.
}{exe:old-div5}{
	$\mathcal{M}_{10} \cap \mathcal{M}_{35} = \{ 0, 70 \}$.
	
	\[ 
		\dfrac{3}{10} + \dfrac{6}{35} = \dfrac{21}{70} + \dfrac{12}{70} = \dfrac{33}{70}
	\]
	
	En outre, $33 = 3 \times 11$ et $70 = 2 \times 5 \times 7$ se décomposent en premiers distincts.
	Lle seul diviseur commun à 33 et 70 est donc 1 et la fraction est irréductible.
}
