...
\label{chap:ensembles-nombres}

\section{Nombres entiers, nombres rationnels}
...
\dfn{notation scientifique}{
	Pour $x\in\DD$ décimal, en choisissant $1 \leq a < 10$, et en permettant $n\in\Z$ entier relatif, l'expression
		\[ x = a \times 10^n, \]
...
\exe{}{
	...
	Si $\frac19 = \frac{a}{10^n}$, alors $10^n = 9a$ et est multiple de $9$.
...
\exe{}{
	...
	\]
	\[
	G = 0,123456789123456789123{\dots} \text{ (nombre d'Amandine\footnotemark) }
	\]
}{exe:dev-to-fraction}{
	\begin{enumerate}[label=\Alph*.]
		\item 
...
\footnotetext{Amandine Inacio (2009 - ), mathématicienne française.}

\nt{
	L'identité $B=9,999{\dots} = 10$, qui semble de prime abord fausse, peut s'expliquer de la façon suivante.
	La différence $\epsilon = 10 - 9,999{\dots} = 0,000{\dots}$ est positive et plus petite que tous les nombres strictement positifs.
	Par conséquent, si $\epsilon$ était non nul, alors sa moitié, $\frac{\epsilon}2$, donnerait un nombre positif strictement plus petit, ce qui contredirait la phrase précédente.
	
	En d'autres termes, il n'existe pas, en mathématiques, de plus petit nombre positif non nul.
	En effet, si $0 \leq \epsilon < x$ pour tout $x>0$, alors $\epsilon = 0$.
}
...
\nt{
	Le fait que le développement décimal d'un nombre soit éventuellement périodique non nul n'implique pas que ce nombre soit non décimal !
	C'est le cas, par exemple, du nombre $B = 9,999{\dots} = 10$ de l'exercice \ref{exe:dev-to-fraction}.
}

\ex{}{
	Le nombre $142,748~210~505~050~505\dots$ (les 05 se répètent indéfiniment) est périodique à partir de la sixième décimale.
...
\exe{, difficulty=2}{
	Démontrer le théorème de Raimon\footnotemark : le nombre 
		\[ x = 0,12345678910111213... \]
	est irrationnel.
}{exe:raimon}{
	Supposons que $x$ soit rationnel.
	D'après le théorème \ref{thm:décimal-périodique}, le développement décimal de $x$ est éventuellement périodique.
	Notons $p$ sa période.
	
	Or, par construction de $x$, son développement décimal comprend tous les entiers naturels.
	En particulier, la partie périodique comprend au moins une très grande puissance de 10.
	
	En prenant une puissance de $10$ suffisamment grande, on peut déduire qu'une période entière est entièrement nulle, et donc que $x$ est décimal.
	Ceci est est faux ; le développement décimal de $x$ est infini. \Large\Lightning
}

\footnotetext{Raimon Joaquim Mendes (2010 - ), mathématicien franco-portugais.}
...
\exe{, difficulty=2}{
	...
	Montrer que les nombres $2\pi, -\pi, \frac{3\pi}4, \pi+3$, et $\frac1{\pi}$ ne sont pas rationnels non plus.
}{exe:irr-stable-pi}{
	Les trois premiers nombres sont de la forme $\frac{a}{b}\pi$ avec $a, b$ entiers (et $b$ non nul).
	À supposer qu'un tel nombre soit rationnel, on aurait $\frac{a}{b}\pi = \frac{p}{q}$, avec $p, q$ entiers.
	Il suivrait alors que $\pi = \frac{pb}{aq} \in\Q$, chose qu'on sait fausse.
	
	En procédant idem, si $\pi+3 = \frac{a}b$ est rationnel, alors $\pi = \frac{a-3b}{b}$ doit l'être aussi.
	
	Finalement, si $\frac{1}{\pi} = \frac{a}b$ est rationnel, alors $a \neq 0$ et $\pi = \frac{b}{a}$ doit aussi l'être.
}
...
\exe{, difficulty=1}{
	...
	\begin{enumerate}
		\item L'amplitude est donnée par $0,000~005 = 5 \times 10^{-6}$.
		\item On multiplie les bornes par 2 puis on soustrait 1, ce qui donne
			\[ 2,23606 < \sqrt5 < 2,23607, \]
		un encadrement d'amplitude $10^{-5}$.
	\end{enumerate}
}
