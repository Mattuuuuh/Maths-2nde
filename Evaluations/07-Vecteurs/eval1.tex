...
\exe{6}{
	Soit $A(2 ; -3)$ un point et $u = \pvec{-2}{4}, v = \dfrac17\pvec{6}{3}$ deux vecteurs.
	...
	\begin{enumerate}
		...
		La réciproque du théorème de Pythagore demande de vérifier l'égalité $BC^2 \stackrel{?}{=} AC^2 + AB^2$, ce que l'on fait calmement car on aime les fractions :
			\[ AC^2 + AB^2 = 20 + \dfrac{45}{49} = \dfrac{20\times49 + 45}{49} = \dfrac{980+45}{49} = \dfrac{1025}{49} = BC^2, \]
...
%\exe{2}{
%	Ruben annonce : \mbox{« le chemin le plus court entre un point $A$ et un point $B$ est la ligne droite »}.
%	Il décide de formuler une version légèrement plus faible de son assertion sous forme vectorielle : 
%	pour n'importe quels vecteurs $u, v$ l'\emph{inégalité triangulaire} tient :
%		\[ \norm{u+v} \leq \norm{u}+\norm{v}. \]
%	Démontrer que Ruben a raison en comparant $\norm{u+v}^2$ et $\left(\norm{u}+\norm{v}\right)^2$.
%}{exe:ineg-triang}{
%	Posons $u = \pvec{a}b$ et $v = \pvec{c}d$.
%	D'une part, 
%		\[ \norm{u+v}^2 = (a+c)^2 + (b+d)^2 = a^2 + c^2 + 2ac + b^2+ d^2 + 2bd. \]
%	D'autre part,
%		\begin{align*}
%		\left(\norm{u}+\norm{v}\right)^2 &= \norm{u}^2 + \norm{v}^2 + 2\norm{u}\cdot\norm{v}, \\
%			&= a^2 + b^2 + c^2 + d^2 + 2\norm{u}\cdot\norm{v}.
...
%	Par conséquent,
%		\[ \left(\norm{u}+\norm{v}\right)^2 - \norm{u+v}^2 = 2\left(\norm{u}\cdot\norm{v} - ac-bd\right) \]
%	Trop dur, il faut Cauchy-Schwarz.
%}
