...
\exe{4, difficulty=1}{
	Considérons la représentation graphique suivante d'une fonction $f$ définie sur \mbox{$\D = ]{-}3,2 ; 2,3[$}.
	
	\begin{center}
		\begin{tikzpicture}[>=stealth]
			\begin{axis}[xmin = -3.2, xmax=2.3, ymin=-2.1, ymax=5.1, axis x line=middle, axis y line=middle, axis line style=->, grid=both,
	...
	\begin{enumerate}
		\item
		$f(-1) \approx 4,8$ et $f(2) \approx 1,7$.
		\item
		Les antécédents de 2 sont environ -2,8 ; 0,5 ; et 2,2.
		En effet, $f(-2,8) \approx f(0,5) \approx f(2,2) \approx 2$
		
		Les antécédents de 2 sont environ -2,2 et -0,5.
		En effet, $f(-2,2) \approx f(-0,5) \approx 4$.
		\item 
		
		\[ \bigset{ x \in \D \tq f(x) \leq 4 } \approx [-3,2 ; -2,2 ] \cup [-0,5 ; 2,3]. \]
	\end{enumerate}
}
...
\exe{2, difficulty=1}{
	Considérons $x, y$ deux nombres réels. Supposons, sans perte de généralité, que $x < y$.
	
	Montrer le \emph{théorème de Léonie} : la moyenne de $x$ et de $y$ est égale à $x$ plus la moitié de la distance entre $x$ et $y$.
}{exe:thm-Leonie}{
